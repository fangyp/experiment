<template>
  <div class="wraper" ref="wraper">
    ...
    <div class="controlPanel" v-if="toolbarVisible">
...
<script>
  ...
  export default{
    ...
    methods:{
      initCanvas() {
		let _this = this
		if (_this.isDrawingMode) {
			this.canvasObj = new fabric.Canvas('canvas',{
	          isDrawingMode: _this.isDrawingMode,
	          selectable: false,
			  selection: false,
			})
			...
			this.canvasObj = new fabric.StaticCanvas('canvas')
		}
	        ...
	        this.canvasObj.on({
			  ...
			  'selection:created': (e) => {
				  if (this.currentTool == 'remove') {
					this.canvasObj.remove(e.target);
			        this.canvasObj.discardActiveObject(); //清楚选中框
				  }
				}
	  ...
	  loadFromJSON(json) {
		  this.canvasObj.clear()
		this.canvasObj.loadFromJSON(json)
	  },
	  // 导出json数据
	  exportData() {
		  return JSON.stringify(this.canvasObj)
	  }
    },
    watch:{
		'drawType': function() {
			this.currentTool = this.drawType
		},
		/*
